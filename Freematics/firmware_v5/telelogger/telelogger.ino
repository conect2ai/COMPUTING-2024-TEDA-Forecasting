...
byte time_rls_update_1_flag = 0x1BB;
byte byte_ypred_virtual = 0x1BC;
byte byte_Y_virtual_0 = 0x1BD;
byte byte_Y_virtual_1 = 0x1BE;
byte time_rls_update_2_flag = 0x1BF;
  ...
  for (byte i = 0; i < sizeof(obdData) / sizeof(obdData[0]); i++) {
    ...
    if (obd.readPID(pid, value)) {
        ...
        if (pid == PID_SPEED) {
          ...
          // struct timeval start_time_rls_update_2, end_time_rls_update_2;
          ...
          // Updating weights
          rls_filter.update(valuef, x_ant);
          ...
          for (int j = 0; j < forecasting-1; j++)
          {
            ...
            // gettimeofday(&start_time_rls_update_2, NULL);
            virtual_filter.update(y_pred_virtual, virtual_input);
            // gettimeofday(&end_time_rls_update_2, NULL);
          ...
          double elapsed_time_rls_update_1 = (end_time_rls_update.tv_sec - start_time_rls_update.tv_sec) +
                      (end_time_rls_update.tv_usec - start_time_rls_update.tv_usec) * 1e-6;

          // double elapsed_time_rls_update_2 = (end_time_rls_update_2.tv_sec - start_time_rls_update_2.tv_sec) +
          //             (end_time_rls_update_2.tv_usec - start_time_rls_update_2.tv_usec) * 1e-6;
          ...
          int microseconds_rls_update_1 = elapsed_time_rls_update_1 * 1000000;
          // int microseconds_rls_update_2 = elapsed_time_rls_update_2 * 1000000;
          ...
          buffer->add(time_rls_update_1_flag, ELEMENT_INT32, &microseconds_rls_update_1, sizeof(microseconds_rls_update_1));
          // buffer->add(time_rls_update_2_flag, ELEMENT_INT32, &microseconds_rls_update_2, sizeof(microseconds_rls_update_2));
          ...
          buffer->add((uint16_t)byte_ypred_virtual | 0x100, ELEMENT_FLOAT, &y_pred_virtual, sizeof(y_pred_virtual));
          buffer->add((uint16_t)byte_Y_virtual_0 | 0x100, ELEMENT_FLOAT, &Y_virtual[0], sizeof(Y_virtual[0]));
          buffer->add((uint16_t)byte_Y_virtual_1 | 0x100, ELEMENT_FLOAT, &Y_virtual[1], sizeof(Y_virtual[1]));
